function moduleProject3() {
  ...
  function buildNav(links) {
    //  ✨ do your magic here
    const docContainer = document.createElement("nav");
    links.forEach((link) => {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    favoriteLanguage.textContent = `Favorite Language: ${favLang.name}`;

    [nameParagraph, idParagraph, dOB, favoriteLanguage].forEach((p) => {
      card.appendChild(p);
    });
  ...
  function buildFooter(footerData) {
    ...
    companyInformation.classList.add("company-info");
    ...
    companyInformation.appendChild(address);
    companyInformation.appendChild(contactEmail);
    ...
    for (let platform in footerData.socialMedia) {
      ...
      socialMed.appendChild(socialLink);
    }
    ...
    footer.appendChild(companyInformation);
